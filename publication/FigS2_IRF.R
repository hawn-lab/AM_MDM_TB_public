...
#### Fig S2: IRFs ####
#IRFs
genes2 <- c("IRF1","IRF7","IRF9", "IRF2", "IRF3", "IRF4", "IRF5", "IRF6", "IRF8") %>% sort()

dat.subset <- dat.format %>% 
  filter(symbol %in% genes2) %>% 
  #Order and add tags
  mutate(symbol = factor(symbol, levels=genes2)) 
fdr.subset <- fdr.format %>% filter(symbol %in% genes2)
...
#### Save ####
ggsave("FigS2_IRF.genes.pdf", ps2, height=6, width=6)
