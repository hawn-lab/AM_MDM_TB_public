...
```{r}
meta <- read_csv("data_raw/MDM-IFN.metadata.csv") %>% 
  mutate(libID = gsub("[.]","-",libID)) %>% 
  #factors
  mutate(IFN = factor(IFN),
         IFN = fct_relevel(IFN, "untreated", after=0L))
```
...
```{r}
counts.raw <- read_tsv("data_raw/MDM-IFN.featurecounts.tsv",
                      skip=1)

#Create simplified names list
name.list <- sub("results/bam/",
                 "", names(counts.raw))
name.list <- sub("_filter.bam",
                 "", name.list)
...
ggplot(meta,
       aes(MEDIAN_CV_COVERAGE, percentaligned)) +
...
```{r tot.seqs, echo=FALSE, message=FALSE, fig.height=8}
tot.seqs <- counts %>% 
  pivot_longer(-ensembl_gene_id, names_to = "libID") %>% 
  group_by(libID) %>% 
  summarise(aligned.seqs = sum(value, na.rm=TRUE), .groups = "drop") %>% 
  #add metadata
  left_join(meta)
```
...
tot.seqs %>% 
  ggplot(aes(x=reorder(libID, aligned.seqs), 
             y=aligned.seqs, fill=IFN))  +
  geom_col() +
  # Add cutoff line
...
plot_pca(counts.pc, meta = meta, vars = c(vars_of_interest, "ptID", "outlier"),
        outlier_group = "IFN") %>% 
  wrap_plots(ncol = 1)
```
...
counts %>% 
  ...
  left_join(key) %>% 
  write_csv("results/MDM-IFN_rare_genes_removed.csv")
```

This removes `r genes-genes.abund` (~ `r round((genes-genes.abund)/genes*100, digits=0)`%) genes. See `results/MDM-IFN_rare_genes_removed.csv` for info on genes removed.
...
plot_pca(dat.pc.abund.norm.voom, meta = meta, vars = c(vars_of_interest, "ptID", "outlier"),
        outlier_group = "IFN") %>% 
  wrap_plots(ncol = 1)
```
